...
- Archive Session (refs #2243) now reloads a cached metadata-only session before mutating `archived`, so the #1558 metadata-only save guard still prevents message wipes without making sidebar archive actions fail with `Archive failed`.
